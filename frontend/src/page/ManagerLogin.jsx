...
export default function ManagerLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Connects to the newly created backend on port 5006 or 5001. We'll set backend to 5001.
      const res = await fetch(`${API_URL}/api/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ staffId, password, role: 'Manager' }),
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-900 via-slate-900 to-black text-white p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white/10 backdrop-blur-xl border border-white/20 p-8 rounded-3xl shadow-[0_0_40px_rgba(79,70,229,0.2)] z-10 transition-all hover:shadow-[0_0_60px_rgba(79,70,229,0.3)]">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl font-extrabold tracking-tight">Manager Portal</h2>
          <p className="text-sm text-indigo-200 mt-2 opacity-80">Strictly Manager Access Only</p>
        </div>

        {error && (
          <div className="bg-red-500/10 border border-red-500/50 text-red-200 text-sm px-4 py-3 rounded-xl mb-6 flex flex-row items-center">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-2">
            <label className="text-xs font-semibold text-indigo-300 uppercase tracking-wider">Manager ID</label>
